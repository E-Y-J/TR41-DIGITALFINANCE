...
@patch(
    "app.auth.decorators.get_user_id_from_claims",
    side_effect=fake_get_user_id_from_claims,
)
@patch(
    "app.auth.decorators.get_token_from_header", side_effect=fake_get_token_from_header
)
...
def test_requires_auth_dev_impersonation_disabled(client):
    ...
    with patch(
        "app.auth.decorators.get_token_from_header",
        side_effect=fake_get_token_from_header,
    ):
        with patch(
            "app.auth.decorators.validate_token", side_effect=fake_validate_token
        ):
            with patch(
                "app.auth.decorators.get_user_id_from_claims",
                side_effect=fake_get_user_id_from_claims,
...
@patch(
    ...
    side_effect=lambda t: (_ for _ in ()).throw(Exception("fail")),
)
@patch(
    "app.auth.decorators.get_token_from_header", side_effect=fake_get_token_from_header
)
