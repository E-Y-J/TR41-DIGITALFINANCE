...
class AnomalyGenerator(BaseGenerator):
    ...
    ANOMALIES: List[Tuple[str, str, float, int, str, str]] = [
        ...
        (
            ...
            5.75,
            5,
            "Duplicate? Same amount same day",
            "duplicate",
        ),
    ...
    def generate(self, db_session) -> int:
        ...
        for merchant, cat_name, amount, days_ago, notes, anomaly_type in self.ANOMALIES:
            ...
            confidence = (
                0.65 if anomaly_type in ["luxury", "legal", "foreign"] else 0.75
            )
...
class GovernmentLegalGenerator(BaseGenerator):
    ...
    def generate(self, db_session) -> int:
        ...
        if not category:
            self.stdout_write(
                "   WARNING: Government & Legal category not found", indent=3
            )
            return 0
        ...
        self.stdout_write(
            f"   Created {count} Government & Legal transactions", indent=3
        )
        return count
